...
*Setup instructions for a Gitian build of Potato Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Potato
...
- [Building Potato Core](#building-potato-core)
...
Clone the git repositories for Potato Core and Gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/potatocoin/potato
```
...
in the Potato Core repository under 'Fetch and build inputs' to install sources which require
manual intervention. Also optionally follow the next step: 'Seed the Gitian sources cache
and offline git repositories' which will fetch the remaining files required for building
offline.

Building Potato Core
----------------

To build Potato Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the Potato Core repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/potato/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/potatocoin/potato
...
For example:
```bash
URL=https://github.com/crowning-/potato.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit potato=${COMMIT} --url potato=${URL} ../potato/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit potato=${COMMIT} --url potato=${URL} ../potato/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit potato=${COMMIT} --url potato=${URL} ../potato/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the potato git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=precise on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../potato/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/potato.git
SIGPATH=/some/root/path/potato-detached-sigs.git

./bin/gbuild --url potato=${BTCPATH},signature=${SIGPATH} ../potato/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/potato-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/potato-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/potato-osx-build.assert
```
...
[potato/gitian.sigs](https://github.com/potatocoin/gitian.sigs/) repository, or if that's not possible to create a pull
request.
